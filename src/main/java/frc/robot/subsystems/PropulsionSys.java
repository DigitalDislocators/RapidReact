...
m_currentPos = new Coordinate();
    ...
    public void mecanumDriveNonlinear(double x, double y, double z, boolean isFieldOriented) {
        if(x < 0) {
            x *= -x;
        }
        else {
    ...
    public void mecanumDrive(double x, double y, double z, boolean isFieldOriented) {
        if(isFieldOriented) {
            mecanumDrive.driveCartesian(y, x, z, gyro.getAngle());
    ...
    public void periodic() {
        ...
        if(timer.hasElapsed(0.1)) {
            timer.reset();
            m_currentPos.setX(m_currentPos.getX() + (getAverageEncoderRate() / Constants.countsPerInch * Math.cos(gyro.getAngle() / 57.2958) * 0.0833333));
            m_currentPos.setY(m_currentPos.getY() + (getAverageEncoderRate() / Constants.countsPerInch * Math.sin(gyro.getAngle() / 57.2958) * 0.0833333));
        }
    ...
    public double getDistance() {
        ...
        rightFrontMtr.getSelectedSensorVelocity() + rightBackMtr.getSelectedSensorVelocity()) * 0.0208333 / Constants.countsPerInch;
    }
    ...
    public double getFeetPerSecond() {
        return -(leftFrontMtr.getSelectedSensorVelocity() + leftBackMtr.getSelectedSensorVelocity() + 
        rightFrontMtr.getSelectedSensorVelocity() + rightBackMtr.getSelectedSensorVelocity()) * 0.2083333 / Constants.countsPerInch;
    }
    ...
    public double getInchesPerSecond() {
        return -(leftFrontMtr.getSelectedSensorVelocity() + leftBackMtr.getSelectedSensorVelocity() + 
        rightFrontMtr.getSelectedSensorVelocity() + rightBackMtr.getSelectedSensorVelocity()) * 2.5 / Constants.countsPerInch;
    }
