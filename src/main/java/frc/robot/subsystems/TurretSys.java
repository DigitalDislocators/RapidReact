...
public class TurretSys extends SubsystemBase {
    ...
    public TurretSys(IndexerSys indexerSys, ShooterSys shooterSys) {
        ...
        sb_turretAngle = Shuffleboard.getTab("DOOFENSHMIRTZ").add("Turret Angle", 0).getEntry();
        sb_isTracking = Shuffleboard.getTab("DOOFENSHMIRTZ").add("Target Locked", false).getEntry();
        sb_trackingEnabled = Shuffleboard.getTab("DOOFENSHMIRTZ").add("Tracking Enabled", false).getEntry();
    ...
    public void joystickRotControl(double x) {
        if(Math.abs(x) > 0.15) {
            x *= x < 0 ? -x : x;
            m_isManualControl = true;
            m_controlAngle += x * Constants.Power.turretSpeedControl;
        }
    }
