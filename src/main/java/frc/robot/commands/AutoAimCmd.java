...
public class AutoAimCmd extends CommandBase {
    ...
    public void execute() {
        ...
        double power = (m_turretSys.getX() * Constants.KP.aim) + m_push;
        ...
        if(Math.abs(m_propulsionSys.getAngleRate()) < 20) {
            if(m_turretSys.getX() > -0.5 && m_turretSys.getX() < + 0.5) {
                m_push = 0;
                m_isFinished = true;
            }
            else {
        ...
        SmartDashboard.putString("Status", "AIMING: " + m_turretSys.getX());
