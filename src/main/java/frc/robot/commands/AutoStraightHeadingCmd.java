...
public class AutoStraightHeadingCmd extends CommandBase {
    ...
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTORS
        ...
        m_counts = ((feet * 12) + inches) * Constants.countsPerInch + m_propulsionSys.getAverageEncoderCounts();
    ...
    public void initialize() {
        ...
        SmartDashboard.putNumber("heading target", m_heading);
        SmartDashboard.putNumber("distance target", m_counts);
    ...
    public void execute() {
        ...
        double turnPower = ((m_heading - m_propulsionSys.getHeading()) * Constants.autoStraightTurnKp) + m_turnPush;
        ...
        double drivePower = ((m_counts - m_propulsionSys.getAverageEncoderCounts()) * Constants.autoStaightKp) + m_drivePush;
        ...
        if(Math.abs(m_propulsionSys.getInchesPerSecond()) < 1) {
            if(m_propulsionSys.getAverageEncoderCounts() > m_counts - (Constants.countsPerInch * 0.5) && m_propulsionSys.getAverageEncoderCounts() < m_counts + (Constants.countsPerInch * 0.5)) {
                m_drivePush = 0;
                m_isFinished = true;
            }
            else {
                if(drivePower < 0) {
                    m_drivePush -= Constants.autoStaightKp * 0.5;
                }
                else {
                    m_drivePush += Constants.autoStaightKp * 0.5;
                }
