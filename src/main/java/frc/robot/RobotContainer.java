...
public class RobotContainer {
  private static RobotContainer m_robotContainer = new RobotContainer();
    ...
    private final JoystickButton m_operatorRightStickPress = new JoystickButton(m_operatorController, 10);
  ...
  */
  private RobotContainer() {
    ...
    Shuffleboard.getTab("DOOFENSHMIRTZ").add("Auto Chooser", m_chooser)
  ...
  private void driverBindingsLogan() {
    ...
    m_driverRight4.whileHeld(new ReverseIndexerCmd(m_indexerSys)).whenReleased(new StopIndexerCmd(m_indexerSys));
    m_driverRight5.whileHeld(new LowGoalShootCmd(m_indexerSys, m_shooterSys)).whenReleased(new StopShooterCmd(m_shooterSys)).whenReleased(new StopIndexerCmd(m_indexerSys));
  ...
  private void operatorBindingsBodi() {
    ...
    m_operatorBButton.whileHeld(new LowGoalShootCmd(m_indexerSys, m_shooterSys)).whenReleased(new StopShooterCmd(m_shooterSys)).whenReleased(new StopIndexerCmd(m_indexerSys));
    ...
    m_operatorYButton.whileHeld(new YEETCmd(m_shooterSys, m_indexerSys)).whenReleased(new StopShooterCmd(m_shooterSys)).whenReleased(new StopIndexerCmd(m_indexerSys));

    m_operatorLeftBumper.whenPressed(new ShooterErrorControlCmd(-Constants.Power.shooterErrorControlInterval, m_shooterSys));
    m_operatorRightBumper.whenPressed(new ShooterErrorControlCmd(Constants.Power.shooterErrorControlInterval, m_shooterSys));
    ...
    m_operatorRightStickPress.whenPressed(new HoldTurretCmd(false, m_turretSys));
    ...
    m_operatorDPadLeft.whileHeld(new RunCommand(
      () -> m_climberSys.rotate(1.0, m_turretSys),
      m_climberSys))
    .whenReleased(new RunCommand(
      () -> m_climberSys.rotate(0.0, m_turretSys),
      m_climberSys));
    ...
    m_turretSys.setDefaultCommand(new RunCommand(
      () -> m_turretSys.joystickRotControl(m_operatorController.getRawAxis(4)),
      m_turretSys));

    m_indexerSys.setDefaultCommand(new RunCommand(
      () -> m_indexerSys.operatorControl(m_operatorController.getRawAxis(1), m_operatorController.getRawAxis(3) > 0.25, m_shooterSys),
      m_indexerSys)
    );
  }
  ...
  @SuppressWarnings("unused")
  public void operatorBindingsTTDemo() {
    m_turretSys.setTrackingEnabled(false);

    m_turretSys.setDefaultCommand(new RunCommand(
      () -> m_turretSys.joystickRotControl(m_operatorController.getRawAxis(4)), m_turretSys)
    );

    m_shooterSys.setDefaultCommand(new RunCommand(
      () -> m_shooterSys.joystickControl(-m_operatorController.getRawAxis(1), m_operatorController.getRawAxis(2) > 0.25), m_shooterSys)
    );

    m_indexerSys.setDefaultCommand(new RunCommand(
      () -> m_indexerSys.triggerControl(m_operatorController.getRawAxis(3) > 0.25), m_indexerSys)
    );
   ...
   * 
   * @param type RumbleType.kLeftRumble shakes and RumbleTyupe.kRightRumble buzzes
   * @param value The 'strength' of the rumbling
   */
  @SuppressWarnings("unused")
  public void setRumble(RumbleType type, double value) {
    if(DriverStation.isTeleopEnabled() && Constants.Rumble.isEnabled) {
